...
function validatePasswordConfirmation(inputName, input) {
  ...
  let isValid = input.value == document.querySelector('#password').value
  console.log(`senha: ${input.value}`)
  console.log(`contrasena: ${document.querySelector('#password').value}`)
  console.log("senha é valida: " + isValid)
  return showOrHideErrorMessage(inputName, isValid, 'invalid');
}
...
function validateBirthday(inputName, input) {
  ...
  // vazio, ele deve ser considerado válido, pois a outra mensagem de erro
  // (sobre ausência/presença) já será exibida.
  let isValid = isEmpty(input.value) || /[0-3][0-9]\/[0-3][0-9]\/\d{4}/.test(input.value)
...
function validatePhone(inputName, input) {
  ...
  return showOrHideErrorMessage(inputName, (isEmpty(input.value) || /\d{2}\d?\d{8}/.test(input.value)), 'invalid');
}
...
document.querySelector('.groups-for-interests > div > div > div > label > input[type="checkbox"]'), addEventListener('change', (e) => {
