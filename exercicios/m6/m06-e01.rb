...
class MySQLExercises1
  ...
  def q_about_query_method
    ...
    @client.query("SELECT * FROM #{USERS} LIMIT 1;").class
  ...
  def q_how_many_tables
    ...
    res.each do |row|
      tables.push row["Tables_in_ecommerce_sample"]
    end
    puts "#{tables}"
    tables
  ...
  def q_title_data_type
    ...
    # máximo permitido.
    quaq = "SHOW COLUMNS FROM #{DB_NAME}.products;"
    puts quaq
    res = self.client.query(quaq)
    res.each do |item|
      if(item["Field"] == "title")
        return item["Type"]
      end
  ...
  def q_categories_primary_key
    ...
    res = self::client.query("SHOW COLUMNS FROM #{DB_NAME}.categories")
    res.each do |i|
      if (i["Key"] == "PRI")
        return i["Field"]
      end
  ...
  def q_reviews_foreign_keys
    ...
    res = self::client.query("SHOW COLUMNS FROM #{DB_NAME}.#{REVIEWS}")
    res.each do |i|
      if(/.+id/.match?(i["Field"]))
        foreign_keys.push i["Field"]
      end
    end
    foreign_keys
  end
  ...
  def q_products_conditions
    ...
    <<-SQL
      SELECT usage_status FROM products
  ...
  def q_products_for_sale
    ...
    <<-SQL
      SELECT COUNT(*) as num_for_sale from products
  ...
  def q_products_discounts
    ...
    <<-SQL
    SELECT id, original_price, CASE WHEN (price != original_price)
    THEN (1 - (price / original_price))
    ELSE 0
    END as discount
    FROM products;
    SQL
  ...
  def q_sales_count_for_product
    ...
    <<-SQL
      ...
      WHERE products.title LIKE "Mochila Puma Training";
    SQL
  end
  ...
  def q_products_without_category
    ...
    <<-SQL
      SELECT COUNT(*) as uncategorized FROM ecommerce_sample.products WHERE category_id is NULL;
    SQL
  end
