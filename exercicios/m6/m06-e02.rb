...
class MySQLExercises2
  ...
  def q_categories_with_parent
    ...
    <<-SQL
    SELECT c.name as parent_name, ca.name as category_name
    FROM categories c
    JOIN categories ca
    ON c.id = ca.parent_category_id
  ...
  def q_buyers_that_are_also_sellers
    ...
    <<-SQL
    SELECT DISTINCT seller_id AS buyer_seller_id
    FROM sales
    WHERE seller_id IN (SELECT DISTINCT buyer_id FROM sales)
  ...
  def q_recently_sold_products_categories
    ...
    FROM sales
    JOIN sales_products ON sales_products.sale_id = sales.id
